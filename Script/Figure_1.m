% construct a simple nonlinear system to identify the difference of linear and
% nonlinear granger causality test on finding the causal relationship of target 
...
% initialize target and predict terms.
phi1 = zeros(len,51); phi2 = zeros(len,51);
x1 = zeros(len,51); x2 = zeros(len,51);
x1(1,:) = repmat(rand(1),[51,1]);
x2(1,:) = repmat(rand(1),[51,1]);

% construct terms. the relationship could be found in [1]
% [1]: 
for j = 1:51
    % 
    e = (j-1)*0.02;
    ...
    for i = 1:len-1       
        ...
        % x2(i+1,j) = (1-a*x2(i,j)^2).^2+c*((sin(jd(i)'*2*pi./365)).^2)*...
        %             ((cos(jd(i)'*2*pi./365)).^2);
    end
    % add error terms.
    x1 = k1+x1;
...
for i = 1:numel(x1(1,:))
    ...
    x10 = x1(:,i);
    x11 = [phi10(1:end-p+1),phi10(2:end-p+2),...
           phi10(3:end-p+3),phi10(4:end-p+4),phi10(5:end-p+5)];
    x12 = x10(p:end);
    x20 = x2(:,i);
    x21 = [phi20(1:end-p+1),phi20(2:end-p+2),...
           phi20(3:end-p+3),phi20(4:end-p+4),phi20(5:end-p+5)];
    x22 = x20(p:end);
    ...
    b = regress(x12,x11);eps0 = var(x12-x11*b);
    b = regress(x12,[x11,x21]);eps1 = var(x12-[x11,x21]*b);
    G_21(i) = log(eps0/eps1);

    b = regress(x22,x21);eps0 = var(x22-x21*b);
    b = regress(x22,[x21,x11]);eps1 = var(x22-[x21,x11]*b);
